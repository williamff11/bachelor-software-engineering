...
def verificaColisaoBola(bola, paleta1, paleta2, bolaDirX):
    som_quando_toca_palhetas = pygame.mixer.Sound("resources/quando_bate.wav")
    if bolaDirX == -1 and paleta1.right == bola.left and paleta1.top < bola.top and paleta1.bottom > bola.bottom:
        som_quando_toca_palhetas.play()
        return -1
...
def verificaPlacar(paleta1, bola, placar, bolaDirX, count):
    # zera a contagem se a bola acerta a borda do jogador
    if bola.left == LARGURA_LINHA:
        som_quando_perde = pygame.mixer.Sound(
            "resources/quando_perde.wav")

        som_quando_perde.play()
        return [0, count]
    elif bola.right == LARGURA_TELA - LARGURA_LINHA:
        placar += 10
        som_quando_ganha = pygame.mixer.Sound(
            "resources/quando_ganha.wav")

        som_quando_ganha.play()
        return [placar, count]
        ...
        if placar == 10:
            quando_faz_10_pontos = pygame.mixer.Sound(
                "resources/quando_faz_10_pontos.wav")

            quando_faz_10_pontos.play()
        return [placar, count]
    else:
        return [placar, count]


def aumenta_velocidade(count, velocidade):
...
def main():
    ...
    while True:  # Loop principal do jogo
        ...
        placar, count = verificaPlacar(paleta1, bola, placar,
                                       bolaDirX, count)

        fps = aumenta_velocidade(count, fps)
